...
<%!
    ParallelGroup getParallelGroup(jakarta.servlet.http.HttpServletRequest request) {
...
%>
<%!
    JobGroupAttrDef getJobGroupAttrDef(jakarta.servlet.http.HttpServletRequest request) {
...
%>
<%!
    JobGroup getJobGroup(jakarta.servlet.http.HttpServletRequest request) {
        // Http request parameter 로 부터 JobGroupAttrDef 객체 생성.
...
<%
	...
	String  returnUrl    = "view_setting?suburl="+suburl;  /* closeThisWin=false 일 경우는 sendRedirect 할 URL을 지정해주어야한다. */
	...
	try {
		...
		if ("add_global_param".equals(cmd)) {
	        ...
	        returnUrl = "view_setting?suburl=calendar&calendarId="+nvl(request.getParameter("calendarId"))+"&yyyy="+nvl(request.getParameter("yyyy"));
            ...
            returnUrl = "form_setting_jobgroup?doreload=yes&parentid="+jobgroup.getParentId(); /* 신규인 경우는 form 창 유지하고 parent 만 reload 함 */
		...
		getMainLog().error("action_setting", e);
