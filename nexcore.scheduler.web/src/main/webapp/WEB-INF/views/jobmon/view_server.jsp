...
<script>
	...
	function openEditFormAgentInfo(agentid) {
   		...
   		window.open("form_server?agentid="+agentid, 'form_server_'+agentid.replace(/-/g, ''), 'width=830,height=400,left='+left+',top='+top+',scrollbars=no, resizable=1').focus();
   	}
    ...
    function openRunningJobExeWin(agentid) {
        window.open("view_server_joblist?agentid="+agentid, 'sever_joblist_'+agentid, 'width=810,height=500,scrollbars=1').focus();
    }

    function openRunningThreadWin(agentid) {
        window.open("view_server_threadlist?agentid="+agentid, 'sever_threadlist_'+agentid, 'width=1000,height=400,scrollbars=1').focus();
    }
    
    function openSystemPropertiesWin(agentid) {
        window.open("view_server_sysproperties?agentid="+agentid, 'sever_prop_'+agentid, 'width=820,height=500,scrollbars=1').focus();
    }

    function openConfigFiles(agentid) {
        window.open("view_server_configfiles?agentid="+agentid, 'sever_configfiles_'+agentid, 'width=1000,height=500,scrollbars=1').focus();
    }
    ...
    function orderby(orderbyCol) {
        ...
        window.location.href = 'view_server?orderby='+orderbyCol+'&orderdir='+orderdir;
    }
</script>
...
<form name="form1" action="action_server" method="post">
...
/*      if (request.getQueryString()==null) {
   ...
   //String reloadUrl = null; // 인코딩된 URL, GET 요청 용
   reloadUrl = request.getRequestURI()
	        .replaceFirst("^/WEB-INF/views", "")  // "/WEB-INF/views" 제거
	        .replaceFirst("\\.jsp$", "");         // ".jsp" 제거
	
	if (request.getQueryString() == null) {
	    reloadUrl += "?dummy=1";
	} else {
	    reloadUrl += "?" + request.getQueryString().replaceAll("&pos_y.*", "");
	}


  System.out.println("Reload URL: " + reloadUrl);
%>
</tbody>
</table>
</form>
