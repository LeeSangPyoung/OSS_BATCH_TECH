...
<script>
    ...
    function doQuery() {
        ...
        if (validateInputData()) {
            document.form1.action="view_jobins";
            document.form1.submit();
        }else {
            return false;
        }
...
%>
            ...
            if (doFullQuery) {
                ...
                httpRequest.open('GET', 'svc_jobins_xml?<%=nvl(request.getQueryString(), "a=a")%>', true);
            }else {
                httpRequest.open('GET', 'svc_jobins_xml?<%=nvl(request.getQueryString(), "a=a")%>&lastmodifytimefrom='+lastModifyTime, true);
            }
            httpRequest.send('');
            httpRequest = null;
        }
