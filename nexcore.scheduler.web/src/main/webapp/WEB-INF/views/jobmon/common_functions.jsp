...
%><%!
	...
	User getUser(jakarta.servlet.http.HttpServletRequest request) {
	    User user = (User)request.getSession().getAttribute("user");
        return user;
	}

	String getUserId(jakarta.servlet.http.HttpServletRequest request) {
	...
	String getUserName(jakarta.servlet.http.HttpServletRequest request) {
	    User user = getUser(request);
	    if (user != null) {
	        return user.getName();
        }
	...
	String getUserIp(jakarta.servlet.http.HttpServletRequest request) {
		return request.getRemoteAddr();
	}

    long getLoginTime(jakarta.servlet.http.HttpServletRequest request) {
        Long loginTime = (Long)request.getSession().getAttribute("loginTime");
        return loginTime.longValue();
    }

    boolean isAdmin(jakarta.servlet.http.HttpServletRequest request) {
        User user = getUser(request);
	    if (user != null) {
	        return user.isAdmin();
        }
    ...
    boolean isOperator(jakarta.servlet.http.HttpServletRequest request) {
        User user = getUser(request);
	    if (user != null) {
    ...
    boolean checkLogin(jakarta.servlet.http.HttpServletRequest request, jakarta.servlet.http.HttpServletResponse response) throws java.io.IOException {
...
/*     String printSelectOption(String optionValue, String s2) {
    ...
    void printTextWithLTGT(byte[] b, int offset, int length, jakarta.servlet.jsp.JspWriter out) throws IOException {
	...
	void printTextWithLTGT(byte[] b, int offset, int length, String charset, jakarta.servlet.jsp.JspWriter out) throws IOException {
		boolean findLTGT = false;
		int beginIndex = offset; 
		int writeLength = 0; 
		for (int i=0; i<length; i++) {
