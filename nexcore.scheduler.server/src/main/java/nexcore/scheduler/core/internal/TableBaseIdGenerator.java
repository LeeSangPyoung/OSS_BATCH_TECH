 ...
 */
	...
	public TableBaseIdGenerator(String idType, SqlSession sqlSession) {
		this.idType         = idType;
		this.sqlSession   = sqlSession;
	}
	
	public TableBaseIdGenerator(String idType, SqlSession sqlSession, int retryCount) {
		this.idType         = idType;
		this.sqlSession   = sqlSession;
	 ...
	 */
	/*
	 * private Entry select(String key) throws SQLException { Map sqlin = new
	 * HashMap(); sqlin.put("idType", idType); sqlin.put("keyName", key);
	 * 
	 * Map result = (Map)sqlMapClient.queryForObject("nbs.scheduler.selectLastSeq",
	 * sqlin);
	 * 
	 * Entry entry = null; if (result != null) { entry = new Entry(); entry.keyName
	 * = (String)result.get("KEY_NAME"); entry.lastSeq =
	 * Integer.parseInt(String.valueOf(result.get("LAST_SEQ")));
	...
	private Entry select(String key) throws SQLException {
	    ...
	    try (SqlSession session = sqlSessionFactory.openSession()) {
	        Map<String, Object> result = session.selectOne("nbs.scheduler.selectLastSeq", sqlin);
	 ...
	 */
	/*
	 * private boolean update(String key, int newLastSeq, String newLastModifyUser,
	 * String newLastModifyTime, Entry oldEntry) throws SQLException { Map sqlin =
	 * new HashMap(); sqlin.put("idType", idType); sqlin.put("keyName", key);
	 * sqlin.put("newLastSeq", newLastSeq); sqlin.put("newLastModifyUser",
	 * newLastModifyUser); sqlin.put("newLastModifyTime", newLastModifyTime);
	 * sqlin.put("oldLastSeq", oldEntry.lastSeq); sqlin.put("oldLastModifyUser",
	 * oldEntry.lastModifyUser); sqlin.put("oldLastModifyTime",
	 * oldEntry.lastModifyTime);
	 * 
	...
	private boolean update(String key, int newLastSeq, String newLastModifyUser, String newLastModifyTime, Entry oldEntry) throws SQLException {
	    ...
	    try (SqlSession session = sqlSessionFactory.openSession()) {
	        int result = session.update("nbs.scheduler.updateLastSeq", sqlin);
	 ...
	 */
	/*
	 * private boolean insert(String key, int lastSeq, String lastModifyUser, String
	 * lastModifyTime) throws SQLException {
	 * 
	 * Map sqlin = new HashMap(); sqlin.put("idType", idType); sqlin.put("keyName",
	 * key); sqlin.put("lastSeq", lastSeq); sqlin.put("lastModifyUser",
	 * lastModifyUser); sqlin.put("lastModifyTime", lastModifyTime); try { int
	 * result = sqlMapClient.update("nbs.scheduler.insertLastSeq", sqlin); return
	 * result > 0; }catch(SQLException e) { Entry entry = select(key); if (entry !=
	 * null) { // insert 실패한 후 select 를 해보니 이미 들어와 있는 경우, PK DUP 이다. 다른 놈이 먼저 INSERT
	 * 한 경우 return false; } // PK DUP 이 아닌데 에러가 발생한 경우, 뭔가 DB 장애일 수 있다. throw e; } }
	 */
	...
	private boolean insert(String key, int lastSeq, String lastModifyUser, String lastModifyTime) throws SQLException {
	    ...
	    try (SqlSession session = sqlSessionFactory.openSession()) {
	        int result = session.insert("nbs.scheduler.insertLastSeq", sqlin);
	        session.commit(); // ✅ MyBatis에서는 수동 commit 필요
	        return result > 0;
	    } catch (PersistenceException e) { // ✅ MyBatis 예외 처리
	        try (SqlSession session = sqlSessionFactory.openSession()) {
	            Entry entry;
	            try {
	                entry = select(key); // ✅ SQLException을 처리
	            } catch (SQLException ex) {
	                throw new PersistenceException("Failed to check existing entry after insert failure", ex);
	            }
