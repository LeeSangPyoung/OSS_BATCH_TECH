...
//	}
	private int updateActivationLog(String procDate, List<String> jobInsIdList) throws SQLException {
	    Map<String, Object> param = new HashMap<>();
	    ...
	    try (SqlSession session = sqlSessionFactory.openSession(true)) { // 자동 커밋 모드
	        return session.update("nbs.scheduler.updateActivationLog", param);
	    }
	}
	...
	private int deleteActivationLog(String procDate) throws SQLException {
	    ...
	    try (SqlSession session = sqlSessionFactory.openSession(true)) { // 자동 커밋 모드
	        return session.update("nbs.scheduler.deleteActivationLog", param);
	    }
	}

	private boolean selectActivationLogExist(String procDate) throws SQLException {
	    try (SqlSession session = sqlSessionFactory.openSession()) {
	        String result = session.selectOne("nbs.scheduler.selectActivationLogExist", procDate);
...
//	}
	...
	private List<String> checkScheduleAndMakeJobInstances(String procDate, Log log) throws SQLException {
	    List<String> jobInstanceIdList = new LinkedList<>();
	    List<JobDefinition> jobdefAll = null;

	    try (SqlSession session = sqlSessionFactory.openSession()) { // 자동 커밋 모드
	        // JobDefinition을 조회하는 쿼리
	        jobdefAll = session.selectList("nbs.scheduler.selectJobDefinitionsByQuery", "");
	        ...
	        for (JobDefinition jobdef : jobdefAll) {
	            try {
	                if (dayScheduler.isScheduledDay(jobdef)) {
	                    jobdef = jobDefinitionManager.getJobDefinitionDeep(jobdef.getJobId()); // 파라미터와 선행조건을 포함해서 Deep으로 다시 읽는다.
	                    try {
	                        // 오늘 수행되어야 할 Job이므로 activate 함. 실패해도 다른건 계속 생성해야 하므로 건건이 TX 처리
	                        session.getConnection().setAutoCommit(false); // 트랜잭션 비자동으로 설정
	                        JobInstance jobins = activator.activate(jobdef, procDate, null, new AdminAuth("DailyActivator", InetAddress.getLocalHost().getHostAddress()));
	                        jobInstanceIdList.add(jobins.getJobInstanceId());
	                        session.commit(); // 커밋
	                    } catch (SQLException e) {
