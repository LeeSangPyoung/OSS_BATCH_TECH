...
public class DefaultJobInstanceIdMaker implements IJobInstanceIdMaker {
	 ...
	 */
	/*
	 * private int getLastJobInstanceIdSeq(String jobId, String procDate) { try {
	 * Map param = new HashMap(); param.put("jobId", jobId); param.put("procDate",
	 * procDate);
	 * 
	 * String maxJobInstanceId =
	 * (String)sqlMapClient.queryForObject("nbs.scheduler.selectLastJobInstanceId",
	 * param);
	 * 
	 * if (Util.isBlank(maxJobInstanceId)) { return 0; }else { return
	 * Integer.parseInt(maxJobInstanceId.substring(jobId.length() +
	 * procDate.length())); }
	 * 
	 * }catch (Exception e) { throw new
	 * SchedulerException("main.jobins.maxid.error", e, jobId, procDate); } }
	 */
	private int getLastJobInstanceIdSeq(String jobId, String procDate) {
	    try {
	        ...
	        String maxJobInstanceId = sqlSession.selectOne("nbs.scheduler.selectLastJobInstanceId", param);
