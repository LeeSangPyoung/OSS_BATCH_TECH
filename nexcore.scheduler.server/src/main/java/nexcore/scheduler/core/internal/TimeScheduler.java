...
//			}
	 ...
	 */
	/*
	 * private boolean insertTimeSchedulerLog(long current) { Map sqlin = new
	 * HashMap(); sqlin.put("systemId", Util.getSystemId());
	 * sqlin.put("lastModifyTime", DateUtil.getTimestampString(current)); try { int
	 * cnt = sqlMapClient.update("nbs.scheduler.insertTimeSchedulerLog", sqlin);
	 * return cnt == 1; // 로그 insert 가 성공하면 true 리턴한다. }catch(Exception e) { // 로그
	 * insert 실패하면 false 리턴한다. if (!(e instanceof SQLException)) {
	 * Util.logError(log, "Insert TimeScheduler log fail.", e); } return false; } }
	 */
	private boolean insertTimeSchedulerLog(long current) {
	    ...
	    try (SqlSession session = sqlSessionFactory.openSession()) {
	        int cnt = session.insert("nbs.scheduler.insertTimeSchedulerLog", sqlin);
	        session.commit(); // ✅ MyBatis에서는 수동 commit 필요
	        return cnt == 1; // 로그 insert가 성공하면 true 리턴
	    } catch (Exception e) {
	        // 로그 insert 실패 시 false 리턴
