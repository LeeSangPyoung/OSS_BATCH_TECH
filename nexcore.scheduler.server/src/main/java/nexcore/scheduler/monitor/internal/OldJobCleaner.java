...
//	}
	private List<String> getToBeDeletedJobIdList(String deleteBaseDate) throws SQLException {
	    try (SqlSession session = sqlSessionFactory.openSession()) { // 자동 커밋 모드
	        return session.selectList("nbs.monitor.selectTargetJobInsIdList", deleteBaseDate);
	    }
	}
...
//	}
	private void insertCleanLog(String date, String deleteBaseDate) throws SQLException {
	    ...
	    try (SqlSession session = sqlSessionFactory.openSession(true)) { // 자동 커밋 모드
	        session.insert("nbs.monitor.insertCleanLog", param);
	    }
	}
...
//	}
	private void updateCleanLog(String date, 
	    ...
	    try (SqlSession session = sqlSessionFactory.openSession(true)) { // 자동 커밋 모드
	        int cnt = session.update("nbs.monitor.updateCleanLog", param);
	        if (cnt != 1) {
	            // 위에서 먼저 insert 한 것을 update 하는 것이므로 1 이 아닐 수 없다.
	            log.warn("[OldJobCleaner] updateCleanLog result : " + cnt);
	        }
...
//				}
				...
				try (SqlSession session = sqlSessionFactory.openSession(true)) { // 자동 커밋 모드
				    try {
				        // NBS_JOB_INS 테이블 삭제
				        delJobInsCnt = session.delete("nbs.monitor.deleteOldJobIns", deleteBaseDateTime);
				        log.info(MSG.get("main.oldjobcleaner.delete.ok", "NBS_JOB_INS", delJobInsCnt));
				    } catch (Exception e) {
				        log.info("[OldJobCleaner] " + MSG.get("com.error.occurred.while", "Delete NBS_JOB_INS"), e);
				    }
				    ...
				    try {
				        // NBS_JOB_INS_OBJ_STORE 테이블 삭제
				        delJobInsObjCnt = session.delete("nbs.monitor.deleteOldJobInsObjStore");
				        log.info(MSG.get("main.oldjobcleaner.delete.ok", "NBS_JOB_INS_OBJ_STORE", delJobInsObjCnt));
				    } catch (Exception e) {
				        log.info("[OldJobCleaner] " + MSG.get("com.error.occurred.while", "Delete NBS_JOB_INS_OBJ_STORE"), e);
				    }
				    ...
				    try {
				        // NBS_JOB_INS_PREJOB 테이블 삭제
				        delJobInsPreJobCnt = session.delete("nbs.monitor.deleteOldJobInsPreJob");
				        log.info(MSG.get("main.oldjobcleaner.delete.ok", "NBS_JOB_INS_PREJOB", delJobInsPreJobCnt));
				    } catch (Exception e) {
				        log.info("[OldJobCleaner] " + MSG.get("com.error.occurred.while", "Delete NBS_JOB_INS_PREJOB"), e);
				    }
				    ...
				    try {
				        // NBS_JOB_INS_TRIGGER 테이블 삭제
				        delJobInsTriggerCnt = session.delete("nbs.monitor.deleteOldJobInsTrigger");
				        log.info(MSG.get("main.oldjobcleaner.delete.ok", "NBS_JOB_INS_TRIGGER", delJobInsTriggerCnt));
				    } catch (Exception e) {
				        log.info("[OldJobCleaner] " + MSG.get("com.error.occurred.while", "Delete NBS_JOB_INS_TRIGGER"), e);
				    }
				    ...
				    try {
				        // NBS_JOB_EXE 테이블 삭제
				        delJobExeCnt = session.delete("nbs.monitor.deleteOldJobExe");
				        log.info(MSG.get("main.oldjobcleaner.delete.ok", "NBS_JOB_EXE", delJobExeCnt));
				    } catch (Exception e) {
				        log.info("[OldJobCleaner] " + MSG.get("com.error.occurred.while", "Delete NBS_JOB_EXE"), e);
				    }
				    ...
				    try {
				        // NBS_JOB_EXE_OBJ_STORE 테이블 삭제
				        delJobExeObjCnt = session.delete("nbs.monitor.deleteOldJobExeObjStore");
				        log.info(MSG.get("main.oldjobcleaner.delete.ok", "NBS_JOB_EXE_OBJ_STORE", delJobExeObjCnt));
				    } catch (Exception e) {
				        log.info("[OldJobCleaner] " + MSG.get("com.error.occurred.while", "Delete NBS_JOB_EXE_OBJ_STORE"), e);
				    }
				    ...
				    try {
				        // NBS_TIMESCH_LOG 테이블 삭제
				        Map<String, String> sqlin = new HashMap<>();
				        sqlin.put("tsDate", deleteBaseDate);
				        delTimeSchLogCnt = session.delete("nbs.monitor.deleteOldTimeSchedulerLog", sqlin);
				        log.info(MSG.get("main.oldjobcleaner.delete.ok", "NBS_TIMESCH_LOG", delTimeSchLogCnt));
				    } catch (Exception e) {
				        log.info("[OldJobCleaner] " + MSG.get("com.error.occurred.while", "Delete NBS_TIMESCH_LOG"), e);
				    }
				    ...
				    try {
				        ...
				        Map<String, String> sqlin = new HashMap<>();
				        sqlin.put("deleteBaseDate", deleteBaseDate);
				        delIdGenBaseCnt = session.delete("nbs.monitor.deleteOldIdGenBase", sqlin);
				        log.info(MSG.get("main.oldjobcleaner.delete.ok", "NBS_IDGEN_BASE", delIdGenBaseCnt));
				    } catch (Exception e) {
				        log.info("[OldJobCleaner] " + MSG.get("com.error.occurred.while", "Delete NBS_IDGEN_BASE"), e);
				    }
				    ...
				    try {
				        // Upload Temp 파일 삭제
				        delUploadTempFileCnt = deleteUploadTempFile();
				        log.info(MSG.get("main.oldjobcleaner.delete.ok", "Upload Temp Dir", delUploadTempFileCnt));
				    } catch (Exception e) {
				        log.info("[OldJobCleaner] " + MSG.get("com.error.occurred.while", "Delete Upload Temp Dir"), e);
				    }
				}
