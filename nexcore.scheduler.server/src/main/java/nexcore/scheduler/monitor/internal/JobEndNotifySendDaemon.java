...
public class JobEndNotifySendDaemon implements Runnable {
	...
	public int _run() throws Exception {
		...
		List<JobNotifySendInfo> sendAllList = sqlSession.selectList("nbs.monitor.selectJobNotifySendList", in);
		...
		for (JobNotifySendInfo sendInfo : sendAllList) {
			...
			if (!"R".equals(sendInfo.getSendState())) {
				...
				int cnt = sqlSession.update("nbs.monitor.updateJobNotifySendListForStart", sendInfo);
...
//					}
					try (SqlSession session = sqlSessionFactory.openSession(false)) { // 수동 커밋 모드
					    for (JobNotifySendInfo info : sendListByDev) {
					        session.update("nbs.monitor.updateJobNotifySendListForEnd", info);
					    }
...
//								}
								try (SqlSession session = sqlSessionFactory.openSession(false)) { // 수동 커밋 모드
								    int cnt = session.delete("nbs.monitor.deleteJobOldNotifySendList", DateUtil.getTimestampString(deleteBaseDate));
								    session.commit(); // 트랜잭션 커밋
								    log.info(MSG.get("main.jobnotify.oldnotify.cleaner.delete.ok", "NBS_NOTIFY_SEND_LIST", cnt)); 
								} 
