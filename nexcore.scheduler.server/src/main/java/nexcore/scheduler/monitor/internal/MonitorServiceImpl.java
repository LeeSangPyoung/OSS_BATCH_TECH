...
import nexcore.scheduler.controller.internal.ControllerServiceImpl;
...
public class MonitorServiceImpl implements IMonitorService {
	...
	public void init() {
		...
		// 백그라운드 스레드를 실행하여 서비스 유지
        Thread monitorThread = new Thread(() -> {
            try {
                while (true) {
                    Thread.sleep(10000); // 10초 대기 (무한 루프)
                }
        ...
        monitorThread.setDaemon(false); // Spring Boot가 종료되지 않도록 유지
        monitorThread.start();
	}
	public void destroy() {
	}
