...
public class ControllerServiceImpl implements IControllerService {
    ...
    @Autowired
    public ControllerServiceImpl(ControllerMain controllerMain) {
	...
	public void init() {
		...
		controllerMain.init();  // <<==== 여기서 실행되도록 추가!
		
		// 백그라운드 스레드를 실행하여 Spring Boot 프로세스를 유지
	    Thread backgroundThread = new Thread(() -> {
	        try {
	            while (true) {
	                Thread.sleep(10000); // 10초 대기 (이 스레드가 종료되면 Spring도 종료될 가능성 있음)
	            }
	    ...
	    backgroundThread.setDaemon(false); // Spring Boot가 종료되지 않도록 유지
