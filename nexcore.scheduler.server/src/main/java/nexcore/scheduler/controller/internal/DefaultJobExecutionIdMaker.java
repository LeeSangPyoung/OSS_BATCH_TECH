...
public class DefaultJobExecutionIdMaker implements IJobExecutionIdMaker {
	 ...
	 */
	/*
	 * private int getLastJobExecutionIdSeq(String jobInstanceId) { try { //
	 * NBS_JOB_INS 테이블 LAST_JOB_EXE_ID 가 최종정보. 처음이라 이 값이 NULL인 경우는 EXE 테이블도 String
	 * maxJobExecutionId = (String)sqlMapClient.queryForObject(
	 * "nbs.scheduler.selectLastJobExeIdOfJobInstance", jobInstanceId); if
	 * (Util.isBlank(maxJobExecutionId)) { maxJobExecutionId =
	 * (String)sqlMapClient.queryForObject("nbs.controller.selectLastExecutionId",
	 * jobInstanceId); }
	 * 
	 * if (Util.isBlank(maxJobExecutionId)) { return 0; }else { return
	 * Integer.parseInt(maxJobExecutionId.substring(maxJobExecutionId.length()-6));
	 * }
	 * 
	 * }catch (Exception e) { throw new
	 * SchedulerException("main.get.max.jobexeid.error", e, jobInstanceId); } }
	 */
	
	private int getLastJobExecutionIdSeq(String jobInstanceId) {
	    try (SqlSession session = sqlSessionFactory.openSession()) {
	        // NBS_JOB_INS 테이블 LAST_JOB_EXE_ID 가 최종정보. 처음이라 이 값이 NULL인 경우는 EXE 테이블도 조회
	        String maxJobExecutionId = session.selectOne("nbs.scheduler.selectLastJobExeIdOfJobInstance", jobInstanceId);
	        
	        if (Util.isBlank(maxJobExecutionId)) {
	            maxJobExecutionId = session.selectOne("nbs.controller.selectLastExecutionId", jobInstanceId);
	        }
