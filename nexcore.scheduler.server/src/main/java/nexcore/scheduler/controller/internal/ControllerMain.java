...
public class ControllerMain {
    ...
    public void init() {
        logger.info("✅ ControllerMain Start 실행됨...");
        ...
        try {
            ...
            // peer 가 아직 미실행중이라면 에러 무시한다. peer 의 start 시에 에러가 체크될 것이다.
        }
...
//			je.setAgentNode(        jobins.getAgentNode()); // 에이전트 이중화를 위해 여기서 에이전트 세팅을 하지않고 orderJobExecutionToAgent 안에서 한다.
	...
	public void orderJobExecutionToAgent(JobInstance jobins, JobExecution jobexe) {
		...
		try {
			...
			// JOB_EXE 테이블에 등록
			/*
			 * try { // INS 테이블의 LAST_JOB_EXE_ID 에 해당하는 EXE 가 불일치가 발행하지 않도록 하기 위해 이렇게 트랜잭션
			 * 처리한다. 2012-11-30. redmine #20073 참조. sqlMapClient.startTransaction();
			 * jobexe.setState(JobExecution.STATE_INIT);
			 * jobExecutionManager.addJobExecution(jobexe);
			 * jobInstanceManager.setLastJobExeId(jobexe.getJobInstanceId(),
			 * jobexe.getJobExecutionId(), jobexe.getAgentNode());
			 * sqlMapClient.commitTransaction(); }catch(Exception e2) { Util.logError(log,
			 * MSG.get("main.jobexe.crud.error", e2, 0, jobexe.getJobExecutionId())); //
			 * JobExe {0} 를 생성하는 중에 에러가 발생하였습니다. error = error == null ? e2 : error; // 위에서
			 * exception 발생시에는 그 e 가 우선이다. }finally { try { sqlMapClient.endTransaction();
			 * }catch (Exception e3){ } }
			 */
			try {
			    sqlSession = sqlSessionFactory.openSession(false); // 트랜잭션 시작 (자동 커밋 방지)
			    jobexe.setState(JobExecution.STATE_INIT);
			    jobExecutionManager.addJobExecution(jobexe);
...
//	}
	public String activateJob(String jobId, String procDate, AdminAuth auth) {
	    try {
	        Util.checkDateYYYYMMDD(procDate);  // 날짜 형식 체크.
