...
//	private String                controllerUrl; // rmi://localhost:8124/BatchController
	...
	/*
	 * private RmiProxyFactoryBean rmiProxyFactoryBean1; private RmiProxyFactoryBean
	 * rmiProxyFactoryBean2;
	 */
	...
	public void init() {
		 ...
		 * rmiProxyFactoryBean1.setServiceUrl("rmi://"+controllerIp+":"+controllerPort+
		 * "/BatchController"); rmiProxyFactoryBean1.setRegistryClientSocketFactory(new
		 * NRMIClientSocketFactory(5000, 30000));
		 * rmiProxyFactoryBean1.afterPropertiesSet(); controllerService =
		 * (IControllerService)rmiProxyFactoryBean1.getObject();
		 * 
		 ...
		 * rmiProxyFactoryBean2.setServiceUrl("rmi://"+controllerIp+":"+controllerPort+
		 * "/BatchMonitor"); rmiProxyFactoryBean2.setRegistryClientSocketFactory(new
		 * NRMIClientSocketFactory(5000, 30000));
		 * rmiProxyFactoryBean2.afterPropertiesSet(); monitorService = (IMonitorService)
		 * rmiProxyFactoryBean2.getObject();
		 */
		try {
            // 기존 RmiProxyFactoryBean 대신 Naming.lookup() 사용
            controllerService = (IControllerService) Naming.lookup("rmi://" + controllerIp + ":" + controllerPort + "/BatchController");
            monitorService = (IMonitorService) Naming.lookup("rmi://" + controllerIp + ":" + controllerPort + "/BatchMonitor");
        } catch (Exception e) {
            e.printStackTrace();
