...
public class BatchCaller {
	...
	private void connect() {
		...
		for (String address : schedulerAddressList) {
			...
			try {
			    controllerService = (IControllerService) Naming.lookup("rmi://" + peerIp + ":" + peerPort + "/BatchController");
			} catch (Exception e) {
			    logError("[BatchCaller] Fail to connect to scheduler RMI: " + peerIp + ":" + peerPort, e);
			}
